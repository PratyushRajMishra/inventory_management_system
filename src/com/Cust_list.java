...
public class Cust_list implements ActionListener {
	JFrame f1;
	JPanel p1,p2;
	JLabel l1;
	JTextField tf1;
	...
	Cust_list()
	{
		f1= new JFrame("OrderUp-Customers List");
		...
		tf1 = new JTextField("Enter Customer ID");
		tf1.setBounds(370,15,425,40);
		tf1.setFont(new Font("Calibri", Font.PLAIN, 20));
		tf1.setVisible(true);
		
		
		btn1= new JButton("SEARCH");
		btn1.setBounds(800, 15, 150, 40);
		...
		p2= new JPanel();
		p2.setBounds(20, 85, 1200, 400);
		p2.setBackground(Color.WHITE);
		p2.setLayout(null);
		p2.setVisible(true);
		
		JScrollPane jsp = new JScrollPane();
		jsp.setBounds(0, 0, 1200, 400);
		jsp.setVisible(true);
		p2.add(jsp);
		...
		Object[] column = { "Customer ID", "Fname", "Lname", "Email", "Gender", "Birth", "Mob", "Address" };
		...
		try {
			...
			java.sql.PreparedStatement pstmt = con1.prepareStatement(
					"SELECT Customer_ID, Fname, Lname, email, Gender, Birth, mob, Address FROM customer ");
		...
		if (e.getSource() == btn1) {
			try {
				...
				PreparedStatement pstmt = con1.prepareStatement(
						"SELECT Customer_ID, Fname, Lname, email, Gender, Birth, mob, Address FROM customer  WHERE Customer_ID=  "
								+ tf1.getText() + " ");
